...
import sendMailbyReportId from "../../utils/sendMailbyReportId";
import toast from "react-hot-toast";
import generatePdfByreportId from "../../utils/generatePdfByreportId";
import fetchReportDetailsAPI from "../../utils/fetchReportDetailsAPI";
...
const DisplayPatientReport = () => {
  ...
  const generatePdf = async (report_id) => {
    ...
    if (result.success) {
      ...
      link.href = result.data.pdfLink.trim();
      link.target = "_blank";
  ...
  const rows = patientReports.map((report) => (
    <Table.Tr key={report.report_id}>
      ...
      <Table.Td>
        ...
        <GrDocumentDownload
          onClick={() => generatePdf(report.report_id)}
          style={{ marginLeft: "10px" }}
        />
        <BsSend
          onClick={() => sendMail(report.report_id)}
          style={{ marginLeft: "10px" }}
        />
      </Table.Td>
    </Table.Tr>
